...
async function loadTargets() {
  ...
  return (connections || []).filter(row =>
    isPaidTarget(row.owner_id, profileById, subscriptionByOwner)
  );
}

async function writeStatus(connection, patch) {
  const environment =
    asText(connection?.metadata?.technical_environment || connection.environment) === "live"
      ? "live"
      : "demo";

  const { error } = await supabase
    .from("tradovate_live_status")
    .upsert({
...
class LiveSession {
  ...
  async resolveContractName(contractId) {
    ...
    if (this.contractNameCache.has(key)) {
      return this.contractNameCache.get(key);
    }

    try {
      const contract = await tradovateGet(
        this.environment,
        `/contract/item?id=${encodeURIComponent(key)}`,
        this.accessToken
      );
      ...
      if (name) {
        this.contractNameCache.set(key, name);
      }
  ...
  rememberPrimaryLeaderFill(report) {
    ...
    const providerTimestampMs =
      new Date(asText(report?.timestamp)).getTime();
    ...
    entries.push({
      ...
      observedAt,
      providerFillId:
        normaliseProviderId(report?.execRefId || report?.id),
    });

    const cutoff =
      observedAt -
      Math.max(COPIER_PRIMARY_MATCH_WINDOW_MS * 4, 10000);

    this.recentPrimaryLeaderFills.set(
      key,
      entries.filter(entry => entry.observedAt >= cutoff)
    );
  }
  ...
  hasPrimaryCoverageForPositionDelta(report) {
    ...
    const entries =
      (this.recentPrimaryLeaderFills.get(key) || [])
        .filter(entry => {
          if (
            now - entry.observedAt >
            Math.max(
              COPIER_PRIMARY_MATCH_WINDOW_MS * 3,
              6000
            )
          ) {
            return false;
    ...
    const coveredQuantity =
      entries.reduce(
        (sum, entry) =>
          sum + Math.abs(num(entry.quantity)),
        0
      );
  ...
  queuePositionDeltaFallback(
    report,
    eventType = "position-delta"
  ) {
    const accountId =
      normaliseProviderId(report?.accountId);

    const contractId =
      normaliseProviderId(report?.contractId);

    const action =
      asText(report?.action).trim();

    const quantity =
      Math.abs(num(report?.lastQty));

    const before =
      num(report?.previousNetPos);

    const after =
      num(report?.currentNetPos);
    ...
    if (this.pendingPositionFallbacks.has(key)) {
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      if (this.hasPrimaryCoverageForPositionDelta(report)) {
        console.log(
          "copier position fallback suppressed by primary Tradovate fill",
          {
            connection_id: this.connection.id,
    ...
    this.pendingPositionFallbacks.set(
      key,
      timer
    );
  }

  async getFollowerPosition(
    providerAccountId,
    contractId
  ) {
    try {
      ...
      const row =
        (Array.isArray(positions) ? positions : [])
          .find(item =>
            normaliseProviderId(item?.accountId) ===
              normaliseProviderId(providerAccountId) &&
            normaliseProviderId(item?.contractId) ===
              normaliseProviderId(contractId)
          );

      return num(
        row?.netPos,
        num(row?.netPosition, 0)
      );
    } catch (error) {
      console.error(
  ...
  async executeFollowerOrder({
    ...
    const targetConnectionId =
      asText(
        followerAccount?.source_connection_id
      ).trim();

    const providerAccountId =
      normaliseProviderId(
        followerAccount?.external_id
      );
    ...
    const targetSession =
      sessions.get(targetConnectionId);

    if (
      !targetSession ||
      !targetSession.accessToken ||
      !targetSession.subscribed
    ) {
      await supabase
        .from("copier_events")
        .insert({
          ...
          status: "blocked",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:session-unavailable`,
          provider_fill_id: providerFillId,
          symbol,
          action,
          quantity,
          message:
            `Follower blocked · ${followerAccount?.name || providerAccountId} session unavailable`,
          payload: {
            reason:
    ...
    if (!accountSpec) {
      await supabase
        ...
        .insert({
          ...
          status: "blocked",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:accountspec-missing`,
          provider_fill_id:
            providerFillId,
          symbol,
          action,
          quantity,
          message:
            `Follower blocked · Tradovate accountSpec unavailable for ${followerAccount?.name || providerAccountId}`,
          payload: {
            reason:
    ...
    if (
      ...
      await supabase
        ...
        .insert({
          ...
          status: "blocked",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:qty-blocked`,
          provider_fill_id:
            providerFillId,
          symbol,
          action,
          ...
          message:
            `Follower blocked · computed quantity ${computedQty} exceeds limits`,
          payload: {
    ...
    if (
      ...
      await supabase
        ...
        .insert({
          ...
          status: "blocked",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:symbol-blocked`,
          provider_fill_id:
            providerFillId,
          symbol,
          action,
          ...
          message:
            `Follower blocked · ${symbol} not allowed for ${followerAccount?.name || providerAccountId}`,
          payload: {
            reason:
              "symbol_not_allowed",
            allowed_symbols:
      ...
      if (
        ...
        await supabase
          ...
          .insert({
            ...
            status: "blocked",
            dedupe_key:
              `route:${group.id}:${providerFillId}:${follower.account_id}:daily-loss`,
            provider_fill_id:
              providerFillId,
            symbol,
            action,
            quantity:
              computedQty,
            message:
              `Follower blocked · daily loss limit reached on ${followerAccount?.name || providerAccountId}`,
            payload: {
              reason:
                "daily_loss_limit",
              max_daily_loss:
    ...
    const routeKey =
      `${group.id}:${follower.account_id}:${providerFillId}`;

    if (
      this.followerOrderInFlight.has(
        routeKey
      )
    ) {
      return;
    ...
    this.followerOrderInFlight.add(
      routeKey
    );
    ...
    const reserveDedupeKey =
      `route:${group.id}:${providerFillId}:${follower.account_id}:reserved`;
      ...
      await supabase
        ...
        .insert({
          ...
          quantity:
            computedQty,
          message:
            `Reserved → ${followerAccount?.name || providerAccountId}: ${action} ${computedQty} ${symbol}`,
          payload: {
            cl_ord_id:
              clientOrderId,
            target_connection_id:
    ...
    if (reserveError) {
      this.followerOrderInFlight.delete(
        routeKey
      );
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          status:
            "success",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:submitted`,
          provider_order_id:
      ...
      await supabase
        ...
        .insert({
          ...
          status: "error",
          dedupe_key:
            `route:${group.id}:${providerFillId}:${follower.account_id}:rejected`,
          provider_fill_id:
            providerFillId,
          symbol,
          action,
          quantity:
            computedQty,
          message:
            `Follower order rejected · ${error instanceof Error ? error.message : String(error)}`,
          payload: {
            cl_ord_id:
              clientOrderId,
            target_connection_id:
  ...
  aggregateFillId(
    ...
    const digest =
      crypto
        .createHash("sha256")
        .update(
          `${groupId}:${providerOrderId || "no-order"}:${[...fillIds].sort().join(",")}`
        )
        .digest("hex")
        .slice(0, 28);
  ...
  async flushLeaderFillBatch(
    batchKey
  ) {
    const batch =
      this.pendingLeaderFillBatches.get(
        batchKey
      );
    ...
    this.pendingLeaderFillBatches.delete(
      batchKey
    );
    ...
    const {
      ...
      await supabase
        .from("copier_groups")
        .select(
          "id,owner_id,name,mode,armed,leader_account_id,max_leader_qty,allowed_symbols"
        )
        .eq(
          "id",
          batch.group.id
    ...
    const aggregateProviderFillId =
      this.aggregateFillId(
        currentGroup.id,
        batch.providerOrderId,
        batch.fillIds
      );
    ...
    if (
      !symbolAllowed(
        batch.symbol,
        currentGroup.allowed_symbols
      ) ||
      aggregateQuantity > maxLeaderQty
    ) {
      const reason =
        !symbolAllowed(
          batch.symbol,
      ...
      await supabase
        ...
        .insert({
          ...
          group_id:
            currentGroup.id,
          leader_account_id:
            currentGroup.leader_account_id,
          event_type:
            "live_leader_fill_aggregate_blocked",
          status:
            "blocked",
          dedupe_key:
            `live:${this.connection.id}:aggregate:${aggregateProviderFillId}:blocked`,
          provider_order_id:
            batch.providerOrderId || null,
          provider_fill_id:
            aggregateProviderFillId,
          symbol:
            batch.symbol || null,
          action:
            batch.action || null,
          ...
          payload: {
            ...
            leader_provider_order_id:
              batch.providerOrderId || null,
            burst_quiet_ms:
              COPIER_FILL_BURST_QUIET_MS,
            burst_max_ms:
              COPIER_FILL_BURST_MAX_MS,
          },
        });
    ...
    console.log(
      "copier leader fill batch ready",
      {
        connection_id:
          this.connection.id,
        group_id:
    ...
    await this.routeLeaderFillToFollowers({
      ...
      leaderEvent: {
        id:
          batch.leaderEvent?.id ||
          aggregateProviderFillId,
        timestamp:
          batch.leaderEvent?.timestamp ||
          nowIso(),
        source_entity_type:
          "aggregated-primary-fill",
        component_fill_ids:
          [...batch.fillIds],
        provider_order_id:
          batch.providerOrderId || null,
  ...
  enqueueLeaderFillBatch({
    ...
    if (!providerOrderId) {
      this.routeLeaderFillToFollowers({
        ...
        console.error(
          "copier immediate follower route failed",
          this.connection.id,
          error instanceof Error
            ? error.message
            : String(error)
    ...
    const batchKey =
      `${group.id}:${providerOrderId}:${asText(action).trim().toLowerCase()}:${asText(symbol).trim().toUpperCase()}`;
    ...
    if (!batch) {
      ...
      this.pendingLeaderFillBatches.set(
        batchKey,
        batch
      );
    }
    ...
    batch.timer =
      setTimeout(() => {
        this.flushLeaderFillBatch(
          batchKey
        ).catch(error =>
          console.error(
            "copier fill batch flush failed",
            this.connection.id,
            error instanceof Error
              ? error.message
              : String(error)
  ...
  async routeLeaderFillToFollowers({
    group,
    leaderEvent,
    providerFillId,
    symbol,
    ...
    const {
      ...
      await supabase
        .from("accounts")
        .select(
          "id,name,external_id,source_connection_id,status,is_archived"
        )
        .in(
          "id",
          followerAccountIds
    ...
    const activeRoutes =
      followers
        .map(follower => {
          ...
          if (
            !followerAccount ||
            followerAccount.is_archived ||
            asText(
              followerAccount.status
            ).toLowerCase() !==
              "active"
          ) {
            return null;
    ...
    if (rejected.length) {
      console.error(
        "copier parallel follower batch had errors",
        {
          connection_id:
            this.connection.id,
          group_id:
      ...
      console.log(
        "copier parallel follower batch complete",
        {
          connection_id:
            this.connection.id,
          group_id:
  ...
  async detectLeaderExecution(
    report,
    eventType
  ) {
    const reportId =
      normaliseProviderId(report?.id);
    ...
    const execType =
      asText(report?.execType)
        .trim()
        .toLowerCase();

    const ordStatus =
      asText(report?.ordStatus)
        .trim()
        .toLowerCase();
    ...
    const inFlightKey =
      `${this.connection.id}:${reportId}`;

    if (
      this.copierEventInFlight.has(
        inFlightKey
      )
    ) {
      return;
    ...
    try {
      ...
      const symbol =
        await this.resolveContractName(
          contractId
        );
      ...
      for (const group of groups) {
        ...
        const dedupeKey =
          `live:${this.connection.id}:fill:${providerFillId || reportId}:group:${group.id}`;
      ...
      console.error(
        "copier leader detection failed",
        this.connection.id,
        error instanceof Error
          ? error.message
          : String(error)
      ...
      setTimeout(
        () =>
          this.copierEventInFlight.delete(
            inFlightKey
          ),
        30000
      ).unref?.();
  ...
  async detectLeaderFill(
    fill,
    eventType
  ) {
    const fillId =
      normaliseProviderId(fill?.id);
    ...
    if (needsOrder) {
      try {
        order =
          await tradovateGet(
            this.environment,
            `/order/item?id=${encodeURIComponent(orderId)}`,
            this.accessToken
          );
      } catch (error) {
        console.error(
  ...
  handleCopierFrame(frame) {
    for (
      const detail
      of propsEvents(frame)
    ) {
      const entityType =
        asText(detail?.entityType)
          .trim()
          .toLowerCase();

      const eventType =
        asText(detail?.eventType)
          .trim();
  ...
  scheduleCopierFillScan(
    ...
    this.copierFillScanTimer =
      setTimeout(() => {
        ...
        this.scanRecentLeaderFills(
          reason
        ).catch(error =>
          console.error(
            "copier REST fill scan failed",
            this.connection.id,
            error instanceof Error
              ? error.message
              : String(error)
  ...
  async scanRecentLeaderFills(
    reason = "user-event"
  ) {
    if (
      this.copierFillScanInFlight ||
      !this.accessToken
    ) {
      return;
    }
  ...
  async scanLeaderPositions(
    ...
    try {
      ...
      if (
        ...
        console.log(
          "copier position baseline ready",
          {
            connection_id:
              this.connection.id,
            positions:
      ...
      const keys =
        new Set([
          ...this.copierPositionSnapshot.keys(),
          ...current.keys()
        ]);
      ...
      for (
        ...
        const before =
          this.copierPositionSnapshot.get(
            key
          ) || {
            accountId:
              key.split(":")[0],
  ...
  startCopierPositionScanner() {
    ...
    this.scanLeaderPositions(
      "initial-position-baseline"
    ).catch(error =>
      console.error(
        "initial copier position scan failed",
        this.connection.id,
        error instanceof Error
          ? error.message
          : String(error)
    ...
    this.copierPositionScanInterval =
      setInterval(() => {
        this.scanLeaderPositions(
          ...
          console.error(
            "periodic copier position scan failed",
            this.connection.id,
            error instanceof Error
              ? error.message
              : String(error)
  ...
  clearHeartbeat() {
    ...
    for (
      const timer
      of this.pendingPositionFallbacks.values()
    ) {
      clearTimeout(timer);
    }
    ...
    for (
      const batch
      of this.pendingLeaderFillBatches.values()
    ) {
      if (batch?.timer) {
        clearTimeout(
          batch.timer
